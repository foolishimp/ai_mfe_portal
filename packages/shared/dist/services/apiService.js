...
class ApiService {
    ...
    async createConfig(configType, data) {
    ...
    async updateConfig(configType, id, data) {
        ...
        try {
            return this.put(endpoint, requestData);
        }
        catch (error) {
            if (error.response?.status === 404) {
    ...
    async submitJobTuple(workorderId, teamconfigId, runtimeconfigId, userId) {
    ...
    async submitJobConfigs(configs, userId, jobConfiguration) {
        ...
        return this.post('/api/v1/jobs/multi-config', requestData); // Use the new endpoint
    }
    async cancelJob(id) {
