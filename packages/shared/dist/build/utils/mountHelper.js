...
export function createMounter(options) {
    ...
    const { domElement, Component, props = {}, appId = '', onUnmount } = options;
    ...
    const enhancedProps = {
        ...props, // Spread the original component props
        configType, // Add normalized configType
        onNavigateTo: props.onNavigateTo || ((id) => {
    ...
    root.render(_jsx(React.StrictMode, { children: _jsxs(StyledEngineProvider, { injectFirst: true, children: [" ", _jsxs(ThemeProvider, { theme: createTheme(), children: [" ", _jsx(CssBaseline, {}), " ", _jsx(Component, { ...enhancedProps })] })] }) }));
 ...
 */
function normalizeConfigType(configType) {
    if (!configType)
        return '';
